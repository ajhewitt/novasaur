; generated by lcc-xr18fl $Version: 4.0 - XR18FL - St. Judy's Compiler $ on Fri May 17 09:26:14 2013
...
;	if (x<y) printf(" <");
	ld4 RL10,'O',sp,(4+16);reg:  INDIRU4(addr)
	jcU4 RL0,RL10,lbdf,L3; GE is flipped test from LT
	ldaD R12,L5; reg:acon
	Ccall _printf
L3:
;	if (x<=y) printf(" <=");
	ld4 RL10,'O',sp,(4+16);reg:  INDIRU4(addr)
	jcU4 rL10,rL0,lbnf,L6 ;GT same as LT but operands reversed
	ldaD R12,L8; reg:acon
	Ccall _printf
L6:
;	if (x==y) printf(" =");
	ld4 RL10,'O',sp,(4+16);reg:  INDIRU4(addr)
	jneU4 rL0,rL10,L9; NE
...
;	if (x>y) printf(" >");
	ld4 RL10,'O',sp,(4+16);reg:  INDIRU4(addr)
	jcU4 rL10,rL0,lbdf,L15 ;LE is flipped test & operands
	ldaD R12,L17; reg:acon
	Ccall _printf
L15:
;	if (x>=y) printf(" >=");
	ld4 RL10,'O',sp,(4+16);reg:  INDIRU4(addr)
	jcU4 rL0,rL10,lbnf,L18; LT=lbnf i.e. subtract B from A and jump if borrow 
...
;	if (x<y) printf(" <");
	ld4 RL10,'O',sp,(4+16);reg:  INDIRI4(addr)
	jcI4 RL0,RL10,lbdf,L24; GE is flipped test from LT
	ldaD R12,L5; reg:acon
	Ccall _printf
L24:
;	if (x<=y) printf(" <=");
	ld4 RL10,'O',sp,(4+16);reg:  INDIRI4(addr)
	jcI4 rL10,rL0,lbnf,L26 ;GT is reveresed operands from LT
	ldaD R12,L8; reg:acon
	Ccall _printf
L26:
;	if (x==y) printf(" =");
...
;	if (x>y) printf(" >");
	ld4 RL10,'O',sp,(4+16);reg:  INDIRI4(addr)
	jcI4 rL10,rL0,lbdf,L32 ;LE is flipped test & operands
	ldaD R12,L17; reg:acon
	Ccall _printf
L32:
;	if (x>=y) printf(" >=");
	ld4 RL10,'O',sp,(4+16);reg:  INDIRI4(addr)
	jcI4 rL0,rL10,lbnf,L34; LT=lbnf i.e. subtract B from A and jump if borrow 
	ldaD R12,L20; reg:acon
	Ccall _printf
L34:
;	printf("\n");
...
;void main( ) {
;scomparulator(10,-10);
	ldI4 Rp1p2,10 ;loading a long integer constant
...
;scomparulator(-11,-11);
	...
	Ccall _scomparulator
;scomparulator(-1,0);
	ldI4 Rp1p2,-1 ;loading a long integer constant
...
;scomparulator(0,0);
	...
	Ccall _scomparulator
;scomparulator(0,1);
	ldI4 Rp1p2,0 ;loading a long integer constant
	ldI4 RL10,1 ;loading a long integer constant
	st4 RL10,'O',sp,(4); arg+f**
	Ccall _scomparulator
;scomparulator(1,1);
...
;scomparulator(1,0);
	...
	Ccall _scomparulator
;comparulator(0,0);
	ldI4 RL10,0 ;loading a long unsigned constant
	cpy4 Rp1p2,RL10; LOADU4(reg)
	st4 RL10,'O',sp,(4); arg+f**
	Ccall _comparulator
;comparulator(0,1);
	ldI4 Rp1p2,0 ;loading a long unsigned constant
	ldI4 RL10,1 ;loading a long unsigned constant
	st4 RL10,'O',sp,(4); arg+f**
	Ccall _comparulator
;comparulator(1,1);
	ldI4 RL10,1 ;loading a long unsigned constant
	...
	Ccall _comparulator
;comparulator(1,0);
	ldI4 Rp1p2,1 ;loading a long unsigned constant
	ldI4 RL10,0 ;loading a long unsigned constant
	st4 RL10,'O',sp,(4); arg+f**
	Ccall _comparulator
;}
...
;   if (flt < 0.0) {
	...
	jcF4 RL0,RL10,lbdf,L51;GEF4(reg,reg) - reverse test
;      *output++ = '-' ;
	ld2 R11,'O',sp,(-4+78);reg:  INDIRP2(addr)
	cpy2 R10,R11
	incm R10,1
...
L52:
;   if (dec_digits < 8) {
	jcI2I r7,8,lbdf,L57; GE is flipped test from LT
...
;	if (s<0){
	...
	jcI2I r11,0,lbdf,L62; GE is flipped test from LT
;		*bptr='-';bptr++;
...
L70:
	jcU2 r1,r7,lbdf,L67 ;LE is flipped test & operands
;		if (flag || r>0||k==1){
...
;	if (s<0){
	ldI4 RL10,0 ;loading a long integer constant
	jcI4 RL0,RL10,lbdf,L76; GE is flipped test from LT
;		*bptr++='-';
	cpy2 R11,R7 ;LOADP2(reg)
	cpy2 R7,R11
	incm R7,1
...
;	if (x<10){
	ld1 R11,'O',sp,(0+6)
	zExt R11 ;CVUI2: widen unsigned char to signed int (zero extend)
	jcI2I r11,10,lbdf,L82; GE is flipped test from LT
;		putc(x+'0');
	ld1 R11,'O',sp,(0+6)
	zExt R11 ;CVUI2: widen unsigned char to signed int (zero extend)
...
;		if (c!='%'){
	cpy1 R11,R7
	zExt R11 ;CVUI2(reg): widen unsigned char to signed int (zero extend)*
	jeqU2I r11,37,L89;EQI2(reg,con)
;			putc(c);
	cpy1 R12,R7
	zExt R12 ;CVUI2(reg): widen unsigned char to signed int (zero extend)*
	Ccall _putc
...
;			switch (c){
	...
	jcI2I r11,99,lbnf,L110  ;LT=lbnf i.e. subtract immedB from A and jump if borrow
	jnI2I r11,108,lbnf,L111; GT reverse  the subtraction
	shl2I R11,1
	ld2 R11,'O',R11,(L112-198);reg:  INDIRP2(addr)
	jumpv r11; JUMPV(reg)
L112:
	dw L96
	dw L94
...
L110:
	ld2 R11,'O',sp,(-5+16) ;reg:INDIRI2(addr)
	jeqU2I r11,88,L97;EQI2(reg,con)
	lbr L91
L111:
	ld2 R11,'O',sp,(-5+16) ;reg:INDIRI2(addr)
	ldaD R10,115; reg:acon
	jeqI2 r11,r10,L95; EQI2(reg,reg)
	jcI2 r11,r10,lbnf,L91; LT=lbnf i.e. subtract B from A and jump if borrow 
L114:
	ld2 R11,'O',sp,(-5+16) ;reg:INDIRI2(addr)
	jeqU2I r11,120,L97;EQI2(reg,con)
	lbr L91
L94:
;					printint(*this++);
...
;    while(Count-- > 0 && v == 0) {
	...
	cpy2 R7,R11
	decm R7,1
	jeqU2I r11,0,L124;EQU2(reg,con)
	jzU2 r1,L121; EQ 0
L124:
;    return v;

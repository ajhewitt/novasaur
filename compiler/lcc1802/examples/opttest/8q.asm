; generated by lcc-xr182b $Version: 5.0 - XR182b $ on Wed May 15 13:39:36 2013
...
;		if (rows[r] && up[r-c+7] && down[r+c]) {
	...
	jzU2 r11,L19; EQ 0
	alu2 R11,R1,R7,sm,smb
	shl2I R11,1
	ld2 R11,'O',R11,(_up+14) ;reg:INDIRI2(addr)
	jzU2 r11,L19; EQ 0
	alu2 R11,R1,R7,add,adc; ADDI2(r,r)
	shl2I R11,1
	ld2 R11,'O',R11,(_down) ;reg:INDIRI2(addr)
	jzU2 r11,L19; EQ 0
;			rows[r] = up[r-c+7] = down[r+c] = 0;
	ld2z R11
	alu2 R10,R1,R7,add,adc; ADDI2(r,r)
	shl2I R10,1
	st2 R11,'O',R10,(_down); ASGNI2(addr,reg)*;
	alu2 R10,R1,R7,sm,smb
	shl2I R10,1
...
;			x[c] = r;
	cpy2 R11,R7
	shl2I R11,1
	st2 R1,'O',R11,(_x); ASGNI2(addr,reg)*;
;			if (c == 7)
	jneU2I r7,7,L23; NE
;				print();
	Ccall _print
...
;	for (r = 0; r < 8; r++){
	incm R1,1
	jcI2I r1,8,lbnf,L15  ;LT=lbnf i.e. subtract immedB from A and jump if borrow
;}
L14:
	release 4; release room for outgoing arguments
	popr r7
...
;   if (flt < 0.0) {
	...
	jcF4 RL0,RL10,lbdf,L47;GEF4(reg,reg) - reverse test
;      *output++ = '-' ;
	ld2 R11,'O',sp,(-4+78);reg:  INDIRP2(addr)
	cpy2 R10,R11
	incm R10,1
...
L48:
;   if (dec_digits < 8) {
	jcI2I r7,8,lbdf,L53; GE is flipped test from LT
...
L66:
	jcU2 r1,r7,lbdf,L63 ;LE is flipped test & operands
;		if (flag || r>0||k==1){
...
;	if (s<0){
	...
	jcI4 RL0,RL10,lbdf,L72; GE is flipped test from LT
;		*bptr++='-';
	cpy2 R11,R7 ;LOADP2(reg) opt1
	incm R7,1
	ldaD R10,45; reg:acon
...
;			switch (c){
	...
	jcI2I r11,99,lbnf,L106  ;LT=lbnf i.e. subtract immedB from A and jump if borrow
	jnI2I r11,108,lbnf,L107; GT reverse  the subtraction
	shl2I R11,1
...
L106:
	ld2 R11,'O',sp,(-5+16) ;reg:INDIRI2(addr)
	jeqU2I r11,88,L93;EQI2(reg,con)
	lbr L87
L107:
	ld2 R11,'O',sp,(-5+16) ;reg:INDIRI2(addr)
	ldaD R10,115; reg:acon
	jeqI2 r11,r10,L91; EQI2(reg,reg)
	jcI2 r11,r10,lbnf,L87; LT=lbnf i.e. subtract B from A and jump if borrow 
L110:
	ld2 R11,'O',sp,(-5+16) ;reg:INDIRI2(addr)
	jeqU2I r11,120,L93;EQI2(reg,con)
	lbr L87
L90:
;					printint(*this++);

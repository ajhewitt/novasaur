%{
/* This is the XR18NG machine description file for the 1802 next generation compiler.
...
//#define INTVARVOLATILE 0x0000	//no register variables in a volatile environment
...
reg: ADDU2(reg,con1)  "?\tcpy2 R%c,R%0\n\tinc R%c\n"  1
reg: ADDP2(reg,con1)  "?\tcpy2 R%c,R%0\n\tinc R%c\n"  1
reg: con0  "\tld2z R%c\n" 1
reg: acon  "\tldaD R%c,%0\n" 1
...
reg:  INDIRU2(addr)     "\tld2 R%c,%0; INDIRU2(addr)\n"  1
...
reg: DIVI4(reg,con2)  "\tshri4I R%c,2; DIVI4(reg,2)\n"   1
...
%%
static void progend(void){
		print(	"\tinclude lcc1802EpiloNG.inc\n"
			"\tinclude IO1802.inc\n");
...
static void progbeg(int argc, char *argv[]) {
        ...
        time_t now;
        struct tm* ptmNow;
        static char rev[] = "$Version: 2.3 - XR18NG - The Birthday Compiler $";
	...
	fprintf(stderr,"Happy Birthday!\n");  //just so I know who's playing
        time(&now);
        ptmNow = localtime(&now);
        printf("; generated by lcc-xr18ng %s on %s\n", rev, asctime(ptmNow));
	...
	print("\tlisting off\n" "\tinclude lcc1802ProloNG.inc\n" "\tlisting on\n");//include the macro package but turn off the listing for now
        ...
        for (i = 0; i < 31; i ++)
                freg2[i] = mkreg("F%d", i, 1, FREG); //makes symbols for 32 float regs - only 0-7 can get used as temps
        ...
        tmask[IREG] = INTTMP; tmask[FREG] = FLTTMP;	//tmask & vmask show what regs can be used for vars and temps
        vmask[FREG] = FLTVAR;
        if (wjrvolatile){
        	fprintf(stderr,"reduced register variables\n");
        	vmask[IREG] = INTVARVOLATILE;
        }else{
...
void gensaveregs(int saveareaoffset){  //routine to save registers extracted from function()
	int i;
	print(	"\talu2I memaddr,sp,(%d),adi,adci\n" "\tsex	memaddr\n",saveareaoffset); //address the save area
	
        for (i = 0; i < NUM_IREGS; i++)	//now we save the int registers
                if (usedmask[IREG]&(1<<i)) {
                        print("\tsavmi r%d\n", i);  //each savvmi saves a reg and adjusts reg x down
                }
...
void genrldregs(int saveareaoffset){ //reload any saved registers from this sp offset
	int i;
	print(	"\talu2I memaddr,sp,(%d),adi,adci\n",saveareaoffset); //address the save area
        ...
        for (i = NUM_IREGS; i >=0 ; i--)	//now we reload the int registers
                if (usedmask[IREG]&(1<<i)) {
                        print("\trldmi r%d,memaddr\n", i);  //each savvmi reloads a reg and adjusts reg x up
                }
}
...
static void function(Symbol f, Symbol caller[], Symbol callee[], int ncalls) {
	...
	for (i = 0; callee[i]; i++) {
        ...
        framesize = roundup(maxargoffset 	//the frame includes the outgoing argument area, 
                + sizefsave + sizeisave 	//the float and int reg save areas,
                + maxoffset, 2);       		// and the area for locals
        printf("%s:\n", f->x.name);

        if (framesize > 2) { //wjr jan 8
                print("\treserve %d\n", framesize-2); //wjr jan 14 - reserve the stack frame 
        }


	if (0!=(usedmask[IREG]+usedmask[FREG]))  //if there are regs to save
		gensaveregs(sizeisave+sizefsave+maxargoffset-1); //generate the code for the location given
...
Interface xr18ngIR = {	//each entry represents size, alignment, out-of-line literals needed
        1, 1, 0,  /* char */
        2, 1, 0,  /* short */
        2, 1, 0,  /* int */
        4, 4, 0,  /* long jan 20*/
...
static char rcsid[] =
	"XR18 Next Generation - LCC backend for 1802 target";

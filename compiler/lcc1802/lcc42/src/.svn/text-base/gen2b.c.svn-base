...
unsigned emitasm(Node p, int nt) {
	...
	Node kids[10];
	static int entryid;
	fprintf(stderr, "%d:",entryid++);
	dumptree(p);	//wjr april 7
	fprint(stderr, "  ");	//wjr april 7
	p = reuse(p, nt);
	rulenum = getrule(p, nt);
	nts = IR->x._nts[rulenum];
	...
	if (IR->x._isinstruction[rulenum] && p->x.emitted)
		fprintf(stderr,"%s", p->syms[RX]->x.name);
	else if (*fmt == '#')
		(*IR->x.emit2)(p);
	else {
		if (*fmt == '?') {
		...
		for ((*IR->x._kids)(p, rulenum, kids); *fmt; fmt++)
			if (*fmt != '%')
				(void)fprintf(stderr,"%c",*fmt);
			else if (*++fmt == 'F')
				fprintf(stderr,"%d", framesize);
			else if (*fmt >= '0' && *fmt <= '9')
				emitasm(kids[*fmt - '0'], nts[*fmt - '0']);
			else if (*fmt >= 'a' && *fmt < 'a' + NELEMS(p->syms))
				fprintf(stderr,"%s",(p->syms[*fmt - 'a']->x.name));
			else
				(void)fprintf(stderr,"%c",*fmt);
	}
	fprint(stderr, "\n");	//wjr april 7
...
void spill(unsigned mask, int n, Node here) {
	...
	here->x.spills = 1;
	usedmask[n] |= mask;
	if (mask&~freemask[n]) { //wjr jan 15
		//fprintf(stderr,"here->x.registered == 0=%d,	here->syms[RX] == NULL=%d,"
		//"here->syms[RX]->x.regnode == NULL=%d,	here->syms[RX]->x.regnode->set != n=%d:%d"
		//"here->syms[RX]->x.regnode->mask&mask) == 0=%d:d\n",
		//	here->x.registered,	here->syms[RX],	here->syms[RX]->x.regnode,
		//	here->syms[RX]->x.regnode->set,n,here->syms[RX]->x.regnode->mask,mask);

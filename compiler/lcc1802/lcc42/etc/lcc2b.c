 ...
 */
 //feb 14 2013 wjr failed changing temp names to be the name of the 1st file.  -savetemps option to keep temporary files
...
extern char *tempname(char *, char *);
...
main(int argc, char *argv[]) {
	...
	for (nf = 0, i = j = 1; i < argc; i++) {
		if (strcmp(argv[i], "-o") == 0) {
			...
			if (argv[i+1] && *argv[i+1] != '-')
				i++;
			continue;
		} else if (strcmp(argv[i], "-savetemps") == 0) { //wjr feb 14 - 3 lines
			savetempflag=1;
			continue;
	...
	if (errcnt == 0 && !Eflag && !Sflag && !cflag && llist[1]) {
		compose(ld, llist[0], llist[1],
			append(outfile ? outfile : "", 0)); //wjr feb 14 was append(outfile ? outfile : concat("a", first(suffixes[4])), 0));
		if (callsys(av))
			errcnt++;
	}
	if (!savetempflag) rm(rmlist); //wjr feb 14
...
static int filename(char *name, char *base) {
	...
	fprintf(stderr,"filename(name=%s,base=%s) {stemp=%s,itemp=%s}\n",name,base,stemp,itemp); //wjr feb 14
	...
	case 0:	/* C source files */
		...
		if (itemp == NULL)
			itemp = tempname(base,first(suffixes[1])); //wjr feb 14
	...
	case 1:	/* preprocessed source files */
		...
		else if ((status = compile(name, stemp?stemp:(stemp=tempname(base,first(suffixes[2]))))) == 0) //wjr feb 14
	...
	case 2:	/* assembly language files */
		...
		if (!Sflag) {
			...
			if (cflag && outfile)
				...
				ofile = tempname(base,first(suffixes[3])); //wjr feb 14
...
char *tempname(char* basename, char *suffix) {
	static int n;
//	char *name = stringf("%s/lcc%d%d%s", tempdir, getpid(), n++, suffix);
	char *name = stringf("%s/%s%s", tempdir, basename, suffix);	//wjr feb 14
fprintf(stderr,"com[0]=%s\n",com[0]);
	if (strstr(com[1], "win32") != NULL)
		name = replace(name, '/', '\\');
